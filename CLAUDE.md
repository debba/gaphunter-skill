...
2. **Explore** — read `package.json`/`Cargo.toml`/`pyproject.toml`, list `src/`, read docs, grep for complaint keywords, and **detect known tools/libraries** (RTK, GitNexus, React Query, Prisma, NextAuth, Tailwind, …) mapping each to the capabilities they provide. The detected list is fed into Phase 3 so findings already covered by an installed tool are marked `status: "present"` or `"partial"` instead of `"missing"`, and surfaced in `meta.detectedTools`.
